...
//   db.oneOrNone('insert into swaps (uuid, token_uuid, eth_address, bnb_address, amount, client_account_uuid, deposit_transaction_hash, created) values (md5(random()::text || clock_timestamp()::text)::uuid, $1, $2, $3, $4, $5, $6, timezone('utc', now())) returning uuid, eth_address, amount, deposit_transaction_hash;', [token.uuid, transaction.from, transaction.client.bnb_address, transaction.amount, transaction.client.uuid, transaction.transactionHash])
//   .then((response) => {
