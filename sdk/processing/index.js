...
const KEY =
  'witness canyon foot sing song tray task defense float bottom town obvious faint globe door tonight alpha battle purse jazz flag author choose whisper';

const processing = {
  finalizeSwaps() {
    db.manyOrNone(
      'select swaps.uuid, swaps.amount, ae.client_bnb_account_uuid from swaps left join client_accounts_eth ae on ae.uuid=swaps.client_account_uuid where swaps.token_uuid = $1 and swaps.deposit_transaction_hash is not null and swaps.transfer_transaction_hash is not null and swaps.processed is null;',
      [HMY_UUID]
    )
      .then(swaps => {
        // for testing only.
        ...
        for (swap of swaps) {
          ...
          db.oneOrNone('select * from client_bnb_accounts where uuid=$1', [
            swap.client_bnb_account_uuid
          ])
            .then(key => {
                ...
                bnb.transfer(
                  ...
                  (err, result) => {
                    if (err) {
                      ...
                      db.none(
                        'update swaps set processed = true where uuid = $1;',
                        [swap.uuid]
                      )
                        .then(() =>
                          console.log(`update swap back-fill ${swap.uuid}`)
                        )
